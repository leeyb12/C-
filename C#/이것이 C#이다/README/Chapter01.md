...
- 애드박(EDVAC)
  - 에니악에서 천재 과학자 존 포이만(John von Neumann)의 손에 거쳐 개선된 구조와 신뢰성을 갖춤
  - 중앙처리장치, 기억장치, 프로그램, 데이터로 이루어진 현대의 모든 컴퓨터의 조상이 되었다. 
- 애드박 이후 1950년대 전까지는 하드웨어 중심으로 발전하였다. 
...
- 프로그래머가 어셈블리어로 프로그래밍 코드를 작성한 후에는 컴파일러라는 소프트웨어를 이용하여 실행 파일을 만들어낸다.
- 프로그래밍 언어(Programming Language) : 어셈블리어처럼 프로그래밍을 작성하기 위해 만들어진 인공 언어체계
...
- 또한 수만 개의 트랜지스터를 하나의 칩에 집적한 마이크로 칩이 발명되면서 급속도로 발전하였다.
- 존 배커스(John Backus)는 IBM에 입사하자마자 어셈블리어의 번역기의 일종인 스피드 코딩 프로젝트에 참여한 후, 1957년 사람의 언어에 가까운 최초의 프로그래밍 언어, **포트란(Fortran)**과 컴파일러를 개발했다.
- 어셈블리어
```bash
Data
var1 DWORD 1
...
- 1964년 MIT공대, AT&Td의 벨 연구소, GE는 멀틱스(Multics)라는 운영체제 개발을 시작했다.
- 멀틱스는 우수한 연구진과 막대한 비용을 투입했지만 실패했다.
- 이 프로젝트에 참여했던 벨 연구소의 데니스 리치(Dennis Ritchie)와 켄 톰슨(KEn Thompson)은 운영체제 개발이라는 경험을 얻었다. 
  - 미니 컴퓨터(냉장고만한) PDP-7d을 활용한 <우주 여행>이라는 게임을 제작
  - 켄 톰슨과 데니스 리치는 유닉스(UNIX)라는 운영체제를 개발했다.
  - 유닉스 PDP-11 버전도 만들었다. 이후 새로운 언어를 만들기로 결심했다.
  - 어셈블리어는 코드 생산성이 너무 낮고 당시 사용하던 B언어(켄 톰슨 제작)은 PDP-11의 새로운 기능을 활용하기 역부족했다.
- 데니스 리치는 B언어의 특징을 물러받은, B언어를 계승한다 하여 C언어가 만들어졌다.
- C언어를 개발한 뒤, 유닉스를 통째로 C언어로 재작성했다.
- 이러한 내용을, AT&T의 벨 연구소는 모든 곳을 공개하여 대학과 기업에 공급했다.
...
- C언어로 작성된 소스코드는 C++ 컴파일러가 컴파일할 수 있지만, C나 C++로 작성된 소스코드는 C#에서 컴파일되지 않는다.
...
- .NET 클래스 라이브러리 : 콘솔, 데스크톱, 웹, 모바일 등에서 동작하는 애플리케이션을 쉽게 개발할 수 있는 API를 제공 (베이직, 포트란, 코볼, 파이썬, C++, C# 등 다양한 언어를 통해 사용할 수 있다.)
...
- C#은 .cs 소스파일을 이용해 실행파일을 만든다.(C컴파일러는 .c를 받고, Java 컴파일러는 .java)
```mermaid
flowchart LR
